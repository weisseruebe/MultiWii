...
static uint8_t  newbaroalt=0,HightChange=0,PSholdChange=0,BaroExit=0;
...
#if defined(Barologic1)
static uint16_t Barologic1Time,AltHoldBlindTimer;
static uint8_t  SoftChange;
#endif

#if defined(Barologic2)
...
static uint16_t cycleTime = 0;     // this is the number in micro second to achieve a full loop, it can differ a little and is taken into account in the PID loop // Crashpilot: No, it is not!
...
void loop () {
...
#if defined(Barologic2)
    if ((rcData[THROTTLE] < MINCHECK && !rcOptions[BOXBARO])  || (rcData[THROTTLE] < MINCHECK && rcOptions[BOXBARO] && conf.activate[BOXARM] > 0 && !rcOptions[BOXARM])) {
#else
    ...
    BaroExit = 0;}
      ...
      if (rcOptions[BOXBARO]) {
        if (!f.BARO_MODE) {
          f.BARO_MODE = 1;
          AltHold = EstAlt;
          SoftChange = Softcount;
          HightChange = 0; AltHoldBlindTimer = 0; Barologic1Time = 0;
      ...
      if (rcOptions[BOXBARO]) {
        if (!f.BARO_MODE) {
          ...
          AltHold = EstAlt;
          AltHold8 = AltHold << 3;
          AltRCTimer0 = 0;                                     // The User has 1sec to center throttle after Althold is engaged
          HightChange = 0; ThrFstTimeCenter = 0;
          initialThrottleHold = rcCommand[THROTTLE];}
      }else f.BARO_MODE = 0;
#endif
    #endif
  ...
  previousTime = currentTime;
    ...
    if (rcOptions[BOXBARO] && f.ARMED)
    {
     int16_t ActualThr = rcCommand[THROTTLE];
     if (currentTime >= AltRCTimer0){                                                                          // 10Hz Loop
     AltRCTimer0 = currentTime+100000;
     Barologic1Time = Barologic1Time + 1;      
     if (abs(ActualThr-LastThrottle) > 15) AltHoldBlindTimer = 0;
     if (HightChange == 1 && SoftChange !=0) SoftChange = SoftChange - 1;
     LastThrottle = ActualThr;}                                                                                // End of 10 Hz Loop
     int16_t ThrDiff = ActualThr - initialThrottleHold;
     if (abs(ThrDiff) < ALT_HOLD_THROTTLE_NEUTRAL_ZONE && HightChange==0) rcCommand[THROTTLE] = constrain(initialThrottleHold + BaroP + BaroD - BaroI,MinThrbaro,MaxThrbaro);
      else{
       HightChange = 1;
       if (ThrDiff < 0 && SoftChange !=0) rcCommand[THROTTLE] = ActualThr - BaroI;                             // Brake Downmovement
       if (AltHoldBlindTimer == 0 && SoftChange == 0) AltHoldBlindTimer = Barologic1Time + AltHoldBlindTime;}
     if (Barologic1Time >= AltHoldBlindTimer && AltHoldBlindTimer !=0) f.BARO_MODE = 0;                        // Set new ref hight
    } 
#endif
    ...
    if (rcOptions[BOXBARO] && f.ARMED)
    {
     int16_t thrdiff = rcData[THROTTLE]-MIDRC;
     int16_t ABSthrdiff = abs(thrdiff);
     if (ABSthrdiff < ALT_HOLD_THROTTLE_NEUTRAL_ZONE && ThrFstTimeCenter == 0) ThrFstTimeCenter = 1;
     if (currentTime >= AltRCTimer0){                                                                          // X Hz Loop
      AltRCTimer0 = currentTime+Barologic2Speed;
      toggle = toggle^1;                                                                                       // toggle = !toggle;
      if (ThrFstTimeCenter == 1){                                                                              // Begin of ThrFstTimeCenter
       if (ABSthrdiff > ALT_HOLD_THROTTLE_NEUTRAL_ZONE){
         HightChange = 1;
         initialThrottleHold = initialThrottleHold + BaroP/100;                                                // Adjust Baselinethr by 1% of BaroP
         if (thrdiff >= 0) AltHold8 = AltHold8 + thrdiff - ALT_HOLD_THROTTLE_NEUTRAL_ZONE;
         if (thrdiff < 0 && toggle == 0) AltHold8 = AltHold8 + thrdiff + ALT_HOLD_THROTTLE_NEUTRAL_ZONE;
        }else HightChange = 0;}                                                                                // End of ThrFstTimeCenter
     }                                                                                                         // End of X Hz Loop
      AltHold = AltHold8 >> 3;      
      if (HightChange == 1) BaroI = 0;
      int16_t VirtualThrottle = initialThrottleHold + BaroP + BaroD - BaroI;
      if (VirtualThrottle <= MINTHROTTLE-300 && HightChange == 1) f.BARO_MODE = 0;                             // has probably landed reset barostuff
      rcCommand[THROTTLE] = constrain(VirtualThrottle,MinThrbaro,MaxThrbaro);
      BaroExit = 1;
    }
#endif

#if defined(Barologic2) && !defined(Rapid_Exit_Barologic2)
    if (!rcOptions[BOXBARO] && f.ARMED && BaroExit == 1){                                                      // Exit Baromode
    int16_t VirtualThrottle = initialThrottleHold + BaroP + BaroD - BaroI;
    if (abs(rcCommand[THROTTLE] - VirtualThrottle) < 5) BaroExit = 0; 
    rcCommand[THROTTLE] = constrain(VirtualThrottle,MinThrbaro,MaxThrbaro);}
#endif
  #endif
