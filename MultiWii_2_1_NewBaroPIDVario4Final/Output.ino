...
/************************************************************************************************************/
  ...
  for (i = 0; i < NUMBER_MOTOR; i++) {
    ...
    if (rcData[THROTTLE] < MINCHECK){
     if (!rcOptions[BOXBARO]) motor[i] = MINCOMMAND;
     else if (conf.activate[BOXARM] > 0 && !rcOptions[BOXARM]) motor[i] = MINCOMMAND;}
 #endif
 #if !(defined(MOTOR_STOP))
    if (rcData[THROTTLE] < MINCHECK){
     if (!rcOptions[BOXBARO]) motor[i] = MINTHROTTLE;
     else if (conf.activate[BOXARM] > 0 && !rcOptions[BOXARM]) motor[i] = MINTHROTTLE;}
 #endif
#endif
#if !(defined(Barologic2))
    if (rcData[THROTTLE] < MINCHECK)
